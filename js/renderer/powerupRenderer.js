...
export class PowerupRenderer {
  ...
  _drawPowerupCard(ctx, powerup, x, y, w, h, hovered) {
    ...
    if (powerup.duration) {
      ...
      this._pixelText(ctx, durationText, x + (w - durationWidth) / 2, y + 102, 0.7);
    } else {
      ctx.fillStyle = '#888888';
      const instantText = 'INSTANT';
      const instantWidth = instantText.length * 4.5;
      this._pixelText(ctx, instantText, x + (w - instantWidth) / 2, y + 102, 0.7);
    }
  ...
  drawActivePowerups(ctx, activePowerups) {
    ...
    activePowerups.forEach(({ powerup, timeLeft }) => {
      const panelW = 80;
      const panelH = 20;
      const x = 8;  // Left side
      const y = offsetY;
      ...
      ctx.fillText(powerup.icon, x + 6, y + 2);

      // Time left (if duration-based)
      if (powerup.duration && timeLeft !== null) {
        ctx.fillStyle = '#ffffff';
        this._pixelText(ctx, `${Math.ceil(timeLeft)}S`, x + 24, y + 6, 1);
      } else {
        ctx.fillStyle = '#88ff88';
        this._pixelText(ctx, 'ACTIVE', x + 24, y + 6, 1);
      }

      // Progress bar for timed power-ups
      if (powerup.duration && timeLeft !== null) {
